...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  18 cyan dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    draw_points_on_circle(window, circle, 18, 'cyan')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  100 slices, thick (thickness=8) brown lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 8
    circle.outline_color = 'brown'
    circle.fill_color = "yellow"
    pizza(window, circle, 100, 'brown', 3)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    title = ('POLYGON test 3:  20 segments with thin green lines')
    window = rg.RoseWindow(500, 400, title)

    circle = rg.Circle(rg.Point(250, 200), 180)
    circle.outline_thickness = 3
    circle.fill_color = 'purple'
    polygon(window, circle, 20, 'green', 3)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4: 120 magenta lines on cyan circle,'
             + ' hops = 57.')
    window = rg.RoseWindow(960, 700, title)

    circle = rg.Circle(rg.Point(480, 350), 300)
    circle.fill_color = 'cyan'
    fancy_polygon(window, circle, 120, 57, 'magenta', 5)
    window.close_on_mouse_click()
